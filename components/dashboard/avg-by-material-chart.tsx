...
interface AverageByMaterialChartProps {
  groups: DashboardAverageGroup[];
}

export function AverageByMaterialChart({ groups }: AverageByMaterialChartProps) {
  ...
  return (
    <section className="rounded-[1.8rem] border border-[color:var(--border)] bg-[color:var(--panel)] p-5 shadow-[0_18px_48px_rgba(12,28,45,0.12)]">
      <div className="flex items-center justify-between gap-3">
        <p className="text-sm font-semibold text-[color:var(--ink)]">Average Time By Material</p>
        <p className="text-xs text-[color:var(--muted)]">Current range</p>
      </div>

      {groups.length === 0 ? (
        <p className="mt-4 text-sm leading-6 text-[color:var(--muted)]">
          No material duration data is available for this range.
        </p>
      ) : (
          ...
          {groups.map((group) => (
            <div key={group.label} className="grid gap-2">
              ...
              <div className="h-3 rounded-full bg-[color:var(--panel-soft)]">
                <div
                  className="h-3 rounded-full bg-[linear-gradient(90deg,rgba(160,77,31,0.9),rgba(23,50,74,0.9))]"
                  style={{
