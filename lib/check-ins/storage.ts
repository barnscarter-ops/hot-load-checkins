...
export async function createSignedBucketUrl(storagePath: string, expiresInSeconds = 300) {
  const env = getServerEnv();
  const supabase = createSupabaseAdminClient();
  ...
  if (error || !data?.signedUrl) {
    throw new Error(`Storage signed URL failed: ${error?.message ?? "No signed URL returned."}`);
  }
